...
(cl-defmacro jove-deftest-ctx-update (name (tt expect &key push prev) &rest body)
  ...
  `(jove-deftest ,(format "update-%s" name) ; `jove-deftest' will add on prefix
     (let ((state (jove-lex-state-make)))
       ,@(mapcar #'(lambda (ctx)
                     `(jove-ctx-stack-push state ,ctx))
                 push)
       (jove-set-tt state ,tt)
       ,@body
...
(cl-defmacro jove-deftest-ctx-before-expr (tt)
  `(jove-deftest ,(intern (format "before-expr-%s" (symbol-name tt)))
     (let ((state (jove-lex-state-make)))
       (jove-set-tt state ,(intern (format "jove-%s" (symbol-name tt))))
...
(jove-deftest brace-is-block-else
  (should (eq t (jove-brace-is-block-p (jove-lex-state-make) jove-ELSE))))

(jove-deftest brace-is-block-semi
  (should (eq t (jove-brace-is-block-p (jove-lex-state-make) jove-SEMI))))

(jove-deftest brace-is-block-eof
  (should (eq t (jove-brace-is-block-p (jove-lex-state-make) jove-EOF))))

(jove-deftest brace-is-block-paren-r
  (should (eq t (jove-brace-is-block-p (jove-lex-state-make) jove-PAREN-R))))
...
(jove-deftest-ctx-update inc-dec-previous-nil
  (jove-INC-DEC nil)
  ;; NOTE: `state' is defined inside the macro
  (jove-set-expr-allowed state nil))
